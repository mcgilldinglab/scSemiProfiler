...
from scvi.module.base import BaseModuleClass, LossRecorder, auto_move_data, PyroBaseModuleClass 
...
class myVAE(BaseModuleClass):
    ...
    def loss(
        ...
        if (type(bulk) == type(None)):
            ...
            return LossRecorder(loss, reconst_loss, kl_local,bulk_loss)# kl_global)
    ...
    def get_reconstruction_loss(self, x, px_rate, px_r, px_dropout, variances, markermask, bulk) -> torch.Tensor:
        ...
        if type(bulk)!= type(None):
            ...
            '''
            predicted_batch_mean = NegativeBinomial(
